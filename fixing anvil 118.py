...
def main():
    ...
    if not vanillaItemsFile:                                #If vanillaItemsFile has not been opened...
        vanillaItemsFile = open('vanillaItems.bin', 'r+b')  #Open vanillaItemsFile for read/write in binary mode
    if not customItemsFile:                                 #If customItemsFile has not been opened...
        customItemsFile = open('customItems.bin', 'r+b')    #Open customItemsFile for read/write in binary mode
    if not moddedItemsFile:                                 #If moddedItemsFile has not been opened...
...
def performSmith():
    ...
    if userChoice == 'LIST':
        ...
        if userChoice in initializedItems.keys():
            while True:
...
def confirm(userChoice):
    while True:                                                                                 #Until user enters a valid choice...
        if (userChoice == 'Y') or (userChoice == 'YES'):                                        #If user chooses a variant of 'yes'...
            ...
            if (userChoice == 'N') or (userChoice == 'NO'):                                     #Else if user chooses a variant of 'no'...
                while True:                                                                     #Until user enters a valid choice...
                    ...
                    if (exitOption == 'Y') or (exitOption == 'YES'):                            #If user chooses a variant of 'yes'...
                        ...
                        if (exitOption == 'N') or (exitOption == 'NO'):                         #If user chooses a variant of 'no'...
...
def regenerateFile():
    ...
    while (userChoice != ''):
        ...
        if (userChoice == 'modded'):                       #If the user chooses to regenerate modded file...
                pickle.dump(moddedItems, moddedItemsFile)       #Regenerate modded file
                
        if (userChoice == 'all'):                            #If the user chooses to regenerate all files...
                pickle.dump(vanillaItems, vanillaItemsFile)     #Regenerate vanilla file
                pickle.dump(moddedItems, moddedItemsFile)       #Regenerate modded file
